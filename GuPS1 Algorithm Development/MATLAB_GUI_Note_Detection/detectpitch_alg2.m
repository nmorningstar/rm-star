function [x, y, note, freq, fund_val, fund_index] = detectpitch_alg2(filename)
%function [x y match_index freq] = detectpitch_alg2(filename)
%
...
while ( and((first_peak_flag < 1),(index <= max_loc)) )
    if( abs(work(index)) >= threshold ) % if the threshold is broken
        ...
        while index_count <= window_indeces %while still inside the window
            while abs(work(index)) >= threshold %while the threshold is still broken
                if abs(work(index)) > abs(work(best_index)) %see if the index is the highest peak
